...
export default function Login({ setUser }) {
  ...
  const submit = async (e) => {
    ...
    try {
      console.log('Submitting to:', `${API_BASE_URL}/api/${isSignup ? 'register' : 'login'}`);
      console.log('Request data:', isSignup ? form : { email: form.email, password: form.password, isAdminLogin });
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/20 p-6 transform transition-all duration-500 hover:scale-105">
          ...
          <div className="flex gap-1 mb-5 p-1 bg-white/5 rounded-xl backdrop-blur-sm">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1 py-2 px-3 rounded-lg font-medium text-sm transition-all duration-300 ${!isAdminLogin && !isSignup
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1 py-2 px-3 rounded-lg font-medium text-sm transition-all duration-300 ${isAdminLogin
          ...
          <div className="space-y-4">
            ...
            <button
              onClick={submit}
              disabled={isLoading}
              className={`w-full py-3 px-4 bg-gradient-to-r from-purple-500 via-pink-500 to-blue-500 text-white font-semibold text-sm rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 flex items-center justify-center gap-2 group ${isLoading ? "opacity-70 cursor-not-allowed" : ""
                }`}
            >
              {isLoading ? (
                <>
